function [x,out] = ada_grad_sm(x0,A,b,mu,x1)
Q = A'*A;
bb = A'*b;
...
etalist = [1,1e-1,1e-1,2e-2,3e-2,1e-2,1e-2];
maxlist = [10000,10000,10000,10000,10000,10000,10000];
stlist = [];  
xk_1 = x0;
for j = 1:length(mulist)
    [xk,st,r] = ada_grad_sm_sub(xk_1,Q,bb,A,b,mulist(j),pmulist(j),maxlist(j),eplist(j),etalist(j));
    stlist(j) = st;
    xk_1 = xk;
end
x = xk;
...
function [x,st,r] = ada_grad_sm_sub(x0,Q,bb, A, b, mu, pmu,mx,epsilon,eta)
n = size(x0,1);
delta = 3e-2;
k = 1;
r = x0*0;
