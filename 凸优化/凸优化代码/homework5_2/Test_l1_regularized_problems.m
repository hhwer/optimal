...
for i = 1:length(w)
    ...
    epsilon = ep(i);
    lambda = 1e-4;
    x2 = BBForSmoothed(A,b,mu,x2,1,lambda,epsilon);
end
t2 = toc;
...
for i = 1:length(w)
    mu = w(i);
    epsilon = ep(i);
    x3 = NesterovForSmoothed(A,b,mu,x3,3,lambda,epsilon);
end
t3 = toc;
...
for i = 1:length(w)
    mu = w(i);
    epsilon = ep(i);
    x4 = Proxgrad(A,b,mu,x4,epsilon);
end
t4 = toc;

% Nesterov's second method for primal
w = [10,1,0.1,0.01,1e-3];
ep = [2e-4,2e-5,2e-6,1e-6,1e-6];
x5 = x0;
tic
for i = 1:length(w)
    mu = w(i);
    epsilon = ep(i);
    x5 = NesterovProxgrad(A,b,mu,x5,epsilon);
end
t5 = toc;
